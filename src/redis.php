...
class Redis
{
    ...
    /**
     * 初始化类
     * @param type $hostname 主机地址
     * @param type $port 主机端口
     * @param type $password 密码
     * @param type $database 数据库
     */
    public function __construct($hostname='127.0.0.1', $port=6379, $password=NULL, $database=0) {
    ...
    {
        $redisCommand = strtoupper(strtolower(trim(str_replace([
                '-',
                '_',
                '.',
            ], ' ', preg_replace('/(?<![A-Z])[A-Z]/', ' \0', $name)))));
        if (in_array(strtoupper($redisCommand), $this->redisCommands)) {
            return $this->executeCommand(strtoupper($name), $params);           
        } else {
     ...
     * @param type $params 命令参数
     * @return type
     */
    private function executeCommand($name, $params = [])
    {
    ...
    /**
     * 通过socket发送命令到redis server
     * @param type $command 命令名称
     * @param type $params 命令参数
     * @return type
     * @throws \Exception
     */
